...
function Signin({ code, cookies, COGNITO_LOGIN_URL, state_prev_url, domain_name_header }) {
    ...
    const fetchData = async () => {
        ...
        if (result.ucid) {
            ...
            //console.log('tenYearFromNow', tenYearFromNow);
            ...
            console.log("pushing to home", state_prev_url);
            // router.push("/"); //router.push("/MyAccount");
            // router.push("/").then(() => router.reload());
            if (state_prev_url != null) {
                if (state_prev_url == '/') {
                    window.location.href = state_prev_url;
...
export const getServerSideProps = async (context) => {
    ...
    var COGNITO_LOGIN_URL = aws_cognito_user_pool_auth_url + "/login?client_id=" + aws_cognito_user_pool_client_id + "&response_type=code&scope=openid&redirect_uri=" + user_url_to_redirect + "/auth/signin";
    console.log("COGNITO_LOGIN_URL", COGNITO_LOGIN_URL)
    ...
    console.log("state_prev_url---------: ", state_prev_url);
    COGNITO_LOGIN_URL += "&state=" + state_prev_url + ""
    if (cookies) {
